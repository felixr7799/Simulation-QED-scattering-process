        ...
        while(len(event)>4):
            ...
            for i in range(2,len(event)):
                for j in range(i+1,len(event)):
                    #distance_measures.append(y_ij(event[i],event[j],q2))
                    distance_measures.append([self.y_ij(event[i],event[j],91.2**2),i,j])
            distance_ordered = sorted(distance_measures,key=itemgetter(0))
            pseudoparticle = event[distance_ordered[0][1]].__add__(event[distance_ordered[0][2]])
            event.pop(distance_ordered[0][1])
            event.pop(distance_ordered[0][2]-1)
            event.append(pseudoparticle)
            list_yij.append(distance_ordered[0][0])
            list_yij.sort()
